class MLP:
    ...
    def __init__(self, layers, activation='tanh'):
        ...
        self.layers = layers
        self.old_weights = [[]]
        for i in range(0, len(self.old_weights)):
            self.old_weights.append([])
        ...
        for k in range(epochs):                                 # Iterate through the epochs
            ...
            for it in range(X.shape[0]):                        # Iterate through the examples in the training set
                ...
                for i in range(len(self.weights)):              # Iterate through the layers
                    ...
                    if len(self.old_weights[i]) > 0:
                        delta_weights += [x * momentum for x in self.old_weights[i]]
                    self.old_weights[i] = delta_weights
